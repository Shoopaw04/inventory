...
if (typeof getActivityLogs === 'undefined') {
    ...
    window.getActivityLogs = async function(filters = {}) {
        ...
        const endpoint = queryString ? `../api/activity_logs.php?${queryString}&_t=${Date.now()}` : `../api/activity_logs.php?_t=${Date.now()}`;
...
document.addEventListener('DOMContentLoaded', async function() {
    try {
        ...
        if (user) {
            ...
            setInterval(async () => {
                try {
                    const response = await fetch('/inventoryazs/api/me.php', {
                        credentials: 'include'
                    });
                    if (!response.ok) {
                ...
                <tbody>
                    ${activities.map(activity => `
                        <tr>
                            ...
                            <td class="terminal-info">
                                ${activity.Terminal_name ? `${activity.Terminal_name} (${activity.Terminal_location})` : 'N/A'}
                            </td>
                        </tr>
                    `).join('')}
                </tbody>
